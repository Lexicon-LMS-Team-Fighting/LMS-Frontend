...
export async function fetchCourses(
  pageNumber: number = 1,
  pageSize: number = 10
): Promise<IPaginatedResponse<ICoursePreview>> {
  try {
    const response = await fetchWithToken<IPaginatedResponse<ICoursePreview>>(
      `${BASE_URL}/course?page=${pageNumber}&pageSize=${pageSize}&include=progress`,
      {}
    );
